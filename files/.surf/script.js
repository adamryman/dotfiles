...
function hintHandler(e){
    ...
    if (pressedKey == 'Enter') {
        ...
        judgeHintNum(readHintInput(hint_num_str));
    } else if (/[a-z]/.test(pressedKey) == false) {
        removeHints();
    } else {
        hint_num_str += pressedKey;
        console.log(hint_num_str);
        var hint_num = readHintInput(hint_num_str);
        if (hint_num * hintchars.length > hint_elems.length + 1) {
...
function readHintInput(str) {
    ...
    for(var h = 0; h < str.length - 1; h++) {
        ...
        console.log(l)
        console.log("num:", num);
        console.log("hintchars_to_int: " + hintchars_to_int[l]);
        console.log("index", h);
        num = (hintchars_to_int[l]) * (hintchars.length) + num;
        console.log(num);
    }
    num = hintchars_to_int[str[str.length-1]] + num;
    console.log(num);
    return num;
}
...
function setHintRules() {
    ...
    sheet.insertRule('a[highlight=hint_elem] {background-color: yellow}', 0);
    sheet.insertRule('a[highlight=hint_active] {background-color: lime}', 0);
}
...
function setHints() {
    ...
    for (var i = 0; i < elems.length; i++) {
        ...
        if ( elem_bottom >= win_top && elem_top <= win_bottom) {
            ...
            var h = hint_elems.length;
            span.innerHTML = "lol";
            span.innerHTML = nextHint(hint_elems.length);// + " " + hint_elems.length;
...
function nextHint(n) {
    ...
    return nextHint(Math.floor(n / hintchars.length)) + hintchars[n % hintchars.length]
}
...
function initKeyBind(e){
    ...
    if( t.nodeType == 1){
        ...
        addKeyBind( 'C-F', function() { hintMode(true); }, e );
        addKeyBind( 'C-c', function() {removeHints(); }, e );
    }
}
...
var hintchars = ["h", "j", "k", "l", "a", "s", "d", "f", "g", "y", "u", "i", "o", "p", "q", "w", "e", "r", "t", "n", "m", "z", "x", "c", "v", "b"]
var hintchars_to_int = {"h": 0, "j": 1, "k": 2, "l": 3, "a": 4, "s": 5, "d": 6, "f": 7, "g": 8, "y": 9, "u": 10, "i": 11, "o": 12, "p": 13, "q": 14, "w": 15, "e": 16, "r": 17, "t": 18, "n": 19, "m": 20, "z": 21, "x": 22, "c": 23, "v": 24, "b": 25}
//var hintchars = ["h", "j", "k", "l", "a"]
